...
namespace NiMathDouble
{
    ...
    {
        m_translation = { transformSingle.m_translation(0), transformSingle.m_translation(1), transformSingle.m_translation(2)};
        m_rotation = HkQuatToNiQuat(transformSingle.m_rotation);
        m_scale = { transformSingle.m_scale(0), transformSingle.m_scale(1), transformSingle.m_scale(2) };
    }

    ::hkQsTransform hkQsTransform::ToSingle() const
    {
        ::hkQsTransform tmp;
        tmp.m_translation = NiPointToHkVector(m_translation.ToSingle());
        tmp.m_rotation = NiQuatToHkQuat(m_rotation.ToSingle());
        tmp.m_scale = NiPointToHkVector(m_scale.ToSingle());
        return tmp;
    }
    ...
    hkQsTransform hkQsTransform_Multiply(const hkQsTransform *a, const hkQsTransform &b)
    {
        ...
        rotatedDir = {rotatedDir.x * a->m_scale.x, rotatedDir.y * a->m_scale.y, rotatedDir.z * a->m_scale.z};

        //tmp.m_translation.setAdd4(a->m_translation, rotatedDir);
        tmp.m_translation = a->m_translation + rotatedDir;
